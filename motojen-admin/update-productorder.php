...
?>
    ...
    <form action="" method="post">
        <div class="flex">
            ...
            $display_status = "SELECT * FROM tbl_billing WHERE id = $id";
            ...
            if ($statscount > 0) {
                while ($statusrow = mysqli_fetch_assoc($statusres)) {
                    ...
                    $payment_status = $statusrow['order_status'];
                    $downpayment = $statusrow['downpayment'];
                }
            }
            ...
            <input type="number" class="box" required maxlength="100" value="<?php echo $downpayment;?>" placeholder="enter username" name="downpayment">
         </div>
                <select name="payment_status" class="select">
                    ...
                    <option value="ready_pickup" <?php if ($payment_status === "ready_pickup") echo "selected"; ?>>Ready to pickup</option>
                    <option value="complete" <?php if ($payment_status === "complete") echo "selected"; ?>>Completed Order</option>
                </select>
...
if (isset($_POST['update_order'])) {
    ...
    // Query to retrieve the total_price and current downpayment
    $selectQuery = "SELECT total_price, downpayment FROM tbl_billing WHERE id = $id";
    $result = mysqli_query($conn, $selectQuery);
    $row = mysqli_fetch_assoc($result);
    $totalAmount = $row['total_price'];
    $currentDownpayment = $row['downpayment'];

    // Check if the downpayment exceeds the total amount
    if ($downpayment > $totalAmount) {
        echo '<script>
            swal({
                title: "Error",
                text: "Downpayment amount exceeds the total amount",
                icon: "error"
            }).then(function() {
                window.location = "productorder.php";
            });
        </script>';
        exit;
    }
    ...
    $updatequery = "UPDATE tbl_billing SET order_status = '$payment_status', downpayment = $downpayment, remaining_bal = $remainingBalance WHERE id = $id";
    ...
    if ($updateres == true) {
        ...
        echo '<script>
            swal({
                ...
                window.location = "update-productorder.php";
            });
        </script>';
        exit;
    }
