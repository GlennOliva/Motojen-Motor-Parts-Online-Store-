...
<section class="form-container">
   ...
   <form action="" method="post">
      ...
      <input type="text" required maxlength="50" name="address" placeholder="enter your address" class="box" oninput="this.value = this.value.replace(/\s/g, '')">
...
<?php
   if(isset($_POST['submit']))
   {
      ...
      //sql query
      $sql = "INSERT INTO tbl_user SET name = '$name' , email = '$email'  , number = $number , password  = '$cpass' , address = '$address'";

      //execute the query
      $res = mysqli_query($conn,$sql);

      //check if the result query is executed
      if($res==true)
      {
         echo '<script>
            swal({
